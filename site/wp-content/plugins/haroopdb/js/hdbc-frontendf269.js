jQuery(document).ready(function($) {
    ...
    function enforceHorizontalLayout($form) {
        ...
        if ($row.length > 0) {
            ...
            $row.find('.hdbc-form-fields-wrap').css({
                'flex': '1',
                'display': 'flex',
                'gap': '5px',
                'align-items': 'flex-end'
            ...
            $row.find('.hdbc-field').css({
                'flex': '1',
                'min-width': '0',
                'margin-bottom': '0'
            });
            
            $row.find('.hdbc-submit-wrap').css({
                'margin-top': '0',
                'flex-shrink': '0'
            });
        }
    ...
    function initForm($form) {
        ...
        $form.on('submit', function(e) {
            ...
            // 기존 에러 스타일 제거
            $currentForm.find('.hdbc-field').removeClass('hdbc-error');
            $currentForm.find('.hdbc-error-message').remove();
            ...
            $currentForm.find('input[required], textarea[required], select[required]').each(function() {
                ...
                if (!value) {
                    const $fieldWrap = $field.closest('.hdbc-field');
                    const labelText = $fieldWrap.find('label').first().text().replace('*', '').trim();
                    
                    $fieldWrap.addClass('hdbc-error');
                    ...
                    if (fieldType === 'email' && !isValidEmail(value)) {
                        ...
                        $fieldWrap.addClass('hdbc-error');
                        $fieldWrap.append('<span class="hdbc-error-message">올바른 이메일 형식이 아닙니다.</span>');
                        isValid = false;
                    }
                    ...
                    if (fieldType === 'tel' && !isValidPhone(value)) {
                        ...
                        $fieldWrap.addClass('hdbc-error');
                        $fieldWrap.append('<span class="hdbc-error-message">올바른 휴대폰 번호 형식이 아닙니다. (예: 010-1234-5678)</span>');
                        isValid = false;
                    }
                }
            });
            ...
            // 2) 라디오 그룹 검증
            $currentForm.find('.hdbc-radio-group').each(function() {
                const $group = $(this);
                ...
                if ($firstRadio.prop('required')) {
                    ...
                    if (!isChecked) {
                        const $fieldWrap = $group.closest('.hdbc-field');
                        const labelText = $fieldWrap.find('.hdbc-field-label').text().replace('*', '').trim();
            ...
            // 3) 체크박스 그룹 검증
            $currentForm.find('.hdbc-checkbox-group').each(function() {
                const $group = $(this);
            ...
            if (!isValid) {
                ...
                // 첫 번째 에러 필드로 스크롤
                const $firstError = $currentForm.find('.hdbc-field.hdbc-error').first();
            ...
            // Ajax 전송
            $.ajax({
                url: hdbc_ajax_obj.ajax_url,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                beforeSend: function() {
                    $currentForm.addClass('hdbc-loading');
                ...
                complete: function() {
                    $currentForm.removeClass('hdbc-loading');
        ...
        $form.find('input[required], textarea[required]').on('blur', function() {
            ...
            if (!$field.val().trim()) {
                ...
                $fieldWrap.removeClass('hdbc-error');
            }
        });
    }
