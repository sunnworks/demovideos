...
(function($) {
    ...
    $(document).ready(function() {
        ...
        function loadFilteredContent(categoryId, subcategoryId, page) {
            page = page || 1;
            var $container = $('.haroop-ba-gallery-container');
            ...
            // AJAX 요청
            $.ajax({
                url: haroopBA.ajaxurl,
                type: 'POST',
                data: {
                    action: 'haroop_ba_filter',
                    category: categoryId,
                    subcategory: subcategoryId,
                    per_page: perPage,
                    paged: page,
                    nonce: haroopBA.nonce || ''
                },
        ...
        $('.ba-login-btn').on('click', function(e) {
            ...
            if (href.indexOf('redirect=') === -1) {
                e.preventDefault();
                window.location.href = haroopBA.loginUrl + '?redirect=' + encodeURIComponent(currentUrl);
            }
        });
        ...
        $(document).on('click', '.ba-pagination a', function(e) {
            ...
            // 스크롤을 갤러리 상단으로 이동
            $('html, body').animate({
                scrollTop: $('.haroop-ba-gallery-container').offset().top - 100
            }, 300);
        });
