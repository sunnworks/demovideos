(function($) {
    ...
    var HaroopPopup = {
        ...
        init: function() {
            ...
            if (typeof window.haroopPopupData === 'undefined') {
                console.error('[HaroopPopup] window.haroopPopupData가 정의되지 않음');
                return;
            }
            
            this.debugLog('haroopPopupData:', window.haroopPopupData);
            this.breakpoint = window.haroop_popup.breakpoint || 992;
        ...
        debugLog: function(message, data) {
            if (this.debugMode && console && console.log) {
                console.log('[HaroopPopup] ' + message, data || '');
            }
        },
        ...
        processPopups: function() {
            ...
            // 모든 팝업 처리
            $('.haroop-popup').each(function() {
                var $popup = $(this);
                var popupId = $popup.data('popup-id');
                var isPCPopup = $popup.hasClass('haroop-pc-popup');
                var isMobilePopup = $popup.hasClass('haroop-mobile-popup');
                ...
                if (self.getCookie('haroop_popup_' + popupId) === 'closed') {
                ...
                // 일반형 팝업 처리
                if ($popup.hasClass('haroop-popup-general')) {
            ...
            // 오버레이 표시
            if (hasActivePopup) {
                $('#haroop-popup-overlay').fadeIn(300);
            }
        },
        
        processGeneralPopupItems: function($popup, popupId, isDesktop) {
            var self = this;
            var $items = $popup.find('.haroop-general-item');
            var visibleItems = [];
            ...
            $items.each(function(index) {
                var $item = $(this);
                var cookieName = 'haroop_popup_' + popupId + '_item_' + index;
                ...
                $item.find('.haroop-popup-today').attr('data-index', index);
                $item.find('.haroop-popup-close').attr('data-index', index);
        ...
        bindEvents: function() {
            ...
            // 팝업 닫기
            $(document).on('click', '.haroop-popup-close', function(e) {
                e.preventDefault();
                self.handleCloseClick($(this));
            });
            
            // 오늘 하루 보지 않기
            $(document).on('click', '.haroop-popup-today', function(e) {
                e.preventDefault();
                self.handleTodayClick($(this));
            });
            
            // 슬라이드 버튼
            $(document).on('click', '.haroop-slide-button', function(e) {
                e.preventDefault();
                self.handleSlideButtonClick($(this));
            });
            
            // 오버레이 클릭
            $(document).on('click', '#haroop-popup-overlay', function(e) {
                if (e.target === this) {
        ...
        handleCloseClick: function($button) {
            var self = this;
            var $popup = $button.closest('.haroop-popup');
            ...
            if ($popup.hasClass('haroop-popup-general')) {
                // 일반형 팝업
                var $item = $button.closest('.haroop-general-item');
                ...
                if (screenWidth < this.breakpoint) {
                    ...
                    $item.fadeOut(200, function() {
                        // 아이템이 숨겨진 후 확인
                        var visibleCount = $popup.find('.haroop-general-item:visible').length;
        ...
        handleTodayClick: function($button) {
            ...
            if ($popup.hasClass('haroop-popup-general') && index !== null && index !== undefined && index !== 'undefined') {
                // 일반형 팝업의 개별 아이템
                var cookieName = 'haroop_popup_' + popupId + '_item_' + index;
                ...
                // 슬라이드형 팝업 전체
                this.setCookie('haroop_popup_' + popupId, 'closed', 1);
                this.debugLog('전체 팝업 쿠키 설정: haroop_popup_' + popupId);
            }
        ...
        checkAllPopupsClosed: function() {
            var self = this;
            // 즉시 확인 후 추가 확인
            var visiblePopups = $('.haroop-popup:visible').length;
            self.debugLog('즉시 체크 - 보이는 팝업: ' + visiblePopups);
            
            if (visiblePopups === 0) {
                self.debugLog('모든 팝업이 닫혔음 - 오버레이 즉시 숨김');
                $('#haroop-popup-overlay').stop(true, true).fadeOut(150);
            } else {
                // 짧은 지연 후 재확인 (애니메이션 완료 대기)
                setTimeout(function() {
                    var visiblePopupsAfter = $('.haroop-popup:visible').length;
        ...
        handleOverlayClick: function() {
            var self = this;
            // 현재 보이는 모든 팝업 찾기
            var $visiblePopups = $('.haroop-popup:visible');
            ...
            if ($visiblePopups.length > 0) {
                ...
                var $closeBtn = $lastPopup.find('.haroop-popup-close:visible').first();
                ...
                $('#haroop-popup-overlay').fadeOut(300);
            }
        },
        ...
        initSlidePopup: function($popup, popupInfo) {
            ...
            var $slides = $popup.find('.haroop-slide');
            var currentSlide = 0;
        ...
        showSlide: function($popup, index) {
            var $slides = $popup.find('.haroop-slide');
            var $buttons = $popup.find('.haroop-slide-button');
        ...
        handleSlideButtonClick: function($button) {
            var popupId = $button.closest('.haroop-popup').data('popup-id');
    ...
    $(document).ready(function() {
        console.log('[HaroopPopup] DOM Ready - Fixed Version');
        HaroopPopup.init();
    });
